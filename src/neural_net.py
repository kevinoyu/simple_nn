...
class Layer(ABC):
    ...
    def __call__(self, x: np.ndarray[GradVal]) -> np.Any:
        return self.forward(x)


class BiasedLinearLayer(Layer):
    def __init__(self, n_input: int, n_output: int) -> GradVal:
        self.shape = (n_input, n_output)

        self.weights: np.ndarray = np.array(
            [[GradVal(2 * random() - 1) for _ in range(n_input)] for _ in range(n_output)]
            if n_output > 1
            else [GradVal(2 * random() - 1) for _ in range(n_input)]
        )

        self.bias: np.ndarray = np.array([GradVal(2 * random() - 1) for _ in range(n_output)])
